...
const Football = async () => {
  ...
  const live_games_res = await fetch(
    "http://localhost:5000/api/v1/football/front/get-livegames",
    {
      cache: "no-cache",
      // next: { revalidate: 6000 },
    }
  );
  const todayFootballGame = await response.json();

  const a_z_data = await a_z_res.json();

  const live_games = await live_games_res.json();
  // console.log(live_games.data);

  // console.log(a_z_data);

  const _response = working_league.response;
  // console.log(response);
  const extractLeagueName = () => {
    let leagues = [];
    let bookmakers_id = [3, 8];

    for (let i = 0; i < _response.length; i++) {
      if (_response[i].league.type != "Cup") {
        let name = _response[i].league.name;
  ...
  const allLeagues = extractLeagueName();
  ...
  return (
    ...
    shadow-lg w-full"
        ...
        <div className="md:pl-5 xl:pl-0 flex flex-col gap-5 w-full  pt-2 md:pt-0">
          ...
          {todayFootballGame.data &&
            todayFootballGame?.data.map((league: [], index: number) => (
              <WeekGames
                data={league}
                date={today}
                key={index}
