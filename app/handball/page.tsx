...
const Handball = async () => {
  const today = moment().format("MM/DD, dddd");

  const betsResponse = await fetch(
    "http://localhost:5000/api/v1/handball/front/get-today-bets",
    {
      cache: "no-cache",
  ...
  const a_z_res = await fetch(
    "http://localhost:5000/api/v1/handball/front/get-a-to-z",
    {
      cache: "no-cache",
      // next: { revalidate: 6000 },
    }
  ...
  const live_games_res = await fetch(
    "http://localhost:5000/api/v1/handball/front/get-livegames",
    {
      cache: "no-cache",
      // next: { revalidate: 6000 },
    }
